// https://leetcode.com//problems/house-robber-iii/#/description		 QuestionId:337
import java.io.*; 
import java.util.*; 
/**
...
Solution: https://discuss.leetcode.com/topic/39834/step-by-step-tackling-of-the-problem
...
private int[] robSub(TreeNode root) {
    ...
    int[] res = new int[2]; // 0th index: without root,  1st index: with root
    res[0] = Math.max(left[0], left[1]) + Math.max(right[0], right[1]); // without root --> with left & right
    res[1] = root.val + left[0] + right[0]; // with root --> without left & rigt
